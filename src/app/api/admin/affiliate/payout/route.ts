...
export async function POST(req: Request) {
  ...
  if (!b.id || !['paid', 'rejected'].includes(String(b.action))) {
    return NextResponse.json({ error: m('invalidParams') }, { status: 400 })
  }

  const sc = createServiceClient()
  const { data: pr } = await sc.from('payout_requests').select('id, status').eq('id', b.id).maybeSingle()
  if (!pr) return NextResponse.json({ error: m('payoutNotFound') }, { status: 404 })
  if (pr.status !== 'requested') return NextResponse.json({ error: m('payoutAlreadyProcessed') }, { status: 409 })
