...
export async function POST(req: Request) {
  ...
  if (code.length !== 6) return NextResponse.json({ error: 'Inserisci il codice a 6 cifre' }, { status: 400 })
  ...
  if (!otp?.payload) return NextResponse.json({ error: 'Nessuna richiesta attiva: ripeti il cambio email.' }, { status: 400 })
  ...
  if (taken && taken.id !== user.id) {
    return NextResponse.json({ error: 'Questa email è già usata da un altro account.' }, { status: 409 })
  }
