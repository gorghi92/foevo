...
export async function POST(req: Request) {
  ...
  if (!(await emailConfigured())) return NextResponse.json({ error: m('emailNotConfigured') }, { status: 503 })

  const { email: raw } = (await req.json().catch(() => ({}))) as { email?: string }
  const newEmail = String(raw || '').trim().toLowerCase()
  if (!newEmail || !newEmail.includes('@')) return NextResponse.json({ error: m('invalidEmail') }, { status: 400 })
  ...
  if (taken) return NextResponse.json({ error: m('emailAlreadyUsed') }, { status: 409 })
  ...
  const { subject, html } = emailChangeOtpEmail(code, newEmail, requestLocale(req))
  const sent = await sendEmail({ to: newEmail, subject, html })
  if (!sent) return NextResponse.json({ error: m('emailSendFailed') }, { status: 502 })
