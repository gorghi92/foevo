...
type Copy = Dictionary['app']['analyses']
...
export default function Report({ initial, premium = false, t, locale }: { initial: any; premium?: boolean; t: Copy; locale: Locale }) {
  ...
  async function share() {
    ...
    try {
      ...
      if (!res.ok) throw new Error(j?.error || t.share.error)
      setSharePath(j.path)
      ...
      <div className="mt-4 flex flex-wrap items-start gap-3">
        <div className="min-w-0 flex-1">
          <div className="mb-2 flex items-center gap-2.5">
            ...
            <span className="label text-brand">{t.eyebrow}</span>
          </div>
        ...
        {data.status === 'done' && (
          sharePath
            ? <button onClick={copyShare} className="btn btn-ghost px-3 py-1.5 text-[13px]" title={shareUrl}>{copied ? <><Check size={14} /> {t.share.copied}</> : <><Copy size={14} /> {t.share.publicLink}</>}</button>
            : <button onClick={share} disabled={sharing} className="btn btn-primary px-3 py-1.5 text-[13px]"><Share2 size={14} /> {sharing ? t.share.creating : t.share.share}</button>
        )}
      </div>
      {sharePath && (
        <div className="mt-2 flex items-center gap-2 rounded-md border border-line bg-bg px-3 py-2 text-xs">
          ...
          <span className="flex-1 truncate">{t.share.activeLabel} <a href={shareUrl || sharePath} target="_blank" rel="noreferrer" className="font-semibold text-brand">{(shareUrl || sharePath).replace(/^https?:\/\//, '')}</a></span>
          <button onClick={copyShare} className="shrink-0 font-semibold text-brand">{copied ? t.share.copiedShort : t.share.copy}</button>
        </div>
      )}

      {data.status === 'processing' && (
        <div className="card mt-4 flex items-center gap-2 p-4 text-sm"><RefreshCw size={16} className="animate-spin" /> {t.processing}</div>
      )}
      {data.status === 'error' && (
        <div className="card mt-4 flex items-center gap-2 p-4 text-sm text-red-500"><AlertTriangle size={16} /> {t.errorPrefix} {data.error}</div>
      )}

      {data.status === 'done' && r && (
        <>
          <div className="mt-6 flex flex-wrap gap-4">
            <Gauge label={t.gaugeConversion} value={r.scores?.conversion ?? 0} primary />
          ...
          <AiPrompt result={r} url={data.url} title={data.title} premium={premium} t={t.aiPrompt} locale={locale} />
          ...
          <div className="mt-5 grid gap-5 lg:grid-cols-[1.3fr_1fr]">
            <div>
              ...
              <p className="mt-2 text-[11px] leading-snug text-muted">
                {/* il grassetto resta nel colore del paragrafo, come prima */}
                <Rich text={t.heatmapNote} strongClass="" />
              </p>
            </div>
            ...
            <div className="flex flex-col gap-4">
              <div className="card p-4">
                ...
                {r.attention?.firstGlance?.length > 0 && (
                  <div className="mt-3 text-[13px]"><b>{t.firstGlance}</b>{' '}
